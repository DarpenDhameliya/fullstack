...
const Productlist = (props) => {
  ...
  let fetchlist = async () => {
    const response = await fetch("http://192.168.0.103:4000/api/party/list", {
      method: "get",
      headers: {
  ...
  let fetchProductlist = async () => {
    const response = await fetch(
      `http://192.168.0.103:4000/api/admin/product/list/${party_nm.id}`,
      {
        method: "get",
        headers: {
          Authorization: `${localStorage.getItem("nodeuser")}`,
  ...
  let handlesenddata = () => {
    // props.history.push({pathname: '/app/addproduct',
    // state: { sellprise: sellprice }})
    // props.history.push('/app/addproduct');
  ...
  let handledelete = async (id) => {
    console.log(id);
    const response = await fetch(
      `http://192.168.0.103:4000/api/admin/product/delete/${id}`,
      {
        method: "delete",
        headers: {
          Authorization: `${localStorage.getItem("nodeuser")}`,
  ...
  return (
    <>
      <Meta pagetitle="product" discription="product details" />
      <Container component="main" maxWidth="xl" className="setcontainer">
        ...
        <Paper className={classes.setProductpaper} elevation={5}>
          ...
          <Select
            ...
            options={partyList}
          />
          {/* <Button variant="contained" size="medium" onClick={fetchProductlist}>
            Add
          </Button> */}
          {/* <button className="btnsub" onClick={fetchProductlist}>Add</button> */}
          <button className="btnsub1" onClick={fetchProductlist}>Add<span></span></button>
        </Paper>
                      ...
                      <TableRow key={index}>
                        ...
                        >
                          <div className="flex-center">
                            <div>
                              <EditIcon onClick={() => handleedit(e._id)} />
                            </div>
                            <div>
                              <DeleteIcon onClick={() => handledelete(e._id)} />
                            </div>
                            <div>
                              <SellIcon onClick={() => handlesell(e)} />
                            </div>
                          </div>
                        </TableCell>
                      </TableRow>
