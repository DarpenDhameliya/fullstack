...
const EditProduct = () => {
  ...
  let fetchlist = async () => {
    const response = await fetch(
      `http://192.168.0.103:4000/api/admin/product/update/${skuidparam.id}`,
      {
        method: "post",
        headers: {
  ...
  const handlesenddata = async () => {
    const response = await fetch(
      `http://192.168.0.103:4000/api/admin/product/finalupdate`,
      {
        method: "put",
        headers: {
    ...
    <>
      <Container component="main" maxWidth="xl" className="setcontainer">
        ...
        <Paper className={classes.setProductpaper} elevation={5}>
          ...
          <Button variant="contained" size="medium" onClick={handlesenddata}>
            update
          </Button>
        </Paper>
      </Container>
