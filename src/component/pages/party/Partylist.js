...
const Partylist = () => {
  ...
  let handlesenddata = async () => {
    console.log(upid)
    if (upid) {
      const response = await fetch(`http://localhost:4000/api/party/edit/${upid}`, {
        method: 'put',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${localStorage.getItem("nodeuser")}`
  ...
  let handledelete = async(id) => {
    const response = await fetch(`http://localhost:4000/api/party/delete/${id}`, {
      method: 'delete',
      headers: {
        'Content-Type': 'application/json',
        "Authorization": `${localStorage.getItem("nodeuser")}`
  ...
  return <>
    ...
    >
      ...
      <Grid container spacing={2}>
        ...
        {partyList.length > 0 && <Grid item xs={12} sm={7} className={classes.setinputlayout}>
