...
router.get('/product/list/:id', async (req, res) => {
  ...
  console.log(id)
  await Product.find({ party_name: id }, { "__v": 0 }).then(e => {
    console.log(e)
...
router.post('/product/update/:id', async (req, res) => {
  ...
  let fetchdata = await Product.findOne({ _id: id },{ "__v": 0 }).then((res) => {
...
if(modified === true){
  return res.status(200).send(successmessage(fetchdata,['data get successfully']))
} else {
...
router.put('/product/finalupdate', async (req, res) => {
  ...
  let fetchdata = await Product.findOne({ _id: skuid },{ "__v": 0 }).then((res) => {
    partyid = res.party_name
    return res
  }).catch((err) => {
    return res.status(500).send(errormessage(err))
...
router.delete('/product/delete/:id', async (req, res) => {
  ...
  let party_product_mismatch = false
  ...
  // let partyfind = await party.find({ user: req.user })
  // partyfind.map((e) => {
  // })
  await party.find({ user: req.user }).then(e => {
    e.map(data => {
